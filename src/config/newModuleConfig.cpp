...
newModuleConfig::newModuleConfig(char * _filename){
  memset(this, 0, sizeof(newModuleConfig));

  strcpy(filename, _filename);
  parsed = false;
}

newModuleConfig::newModuleConfig(char * _filename, ModuleConfig * oC){
  memset(this, 0, sizeof(newModuleConfig));
...
newModuleConfig::~newModuleConfig(){
  loggerf(DEBUG, "Destructor newModuleConfig %s", this->filename);
...
void newModuleConfig::newModule(uint8_t file, uint8_t connections){
  header = (struct configStruct_Unit *)_calloc(1, struct configStruct_Unit);
  header->Module = file;
...
void printNew_link(char ** debug, struct configStruct_RailLink link){
...
void printNew_Node(struct configStruct_Node node){
  char debug[300];
  char * debugptr = debug;
...
void printNew_Block(struct configStruct_Block block){
  ...
  printNew_link(&debugptr, block.next);
  printNew_link(&debugptr, block.prev);
...
void printNew_Switch(struct configStruct_Switch Switch){
  char debug[300];
  char * debugptr = debug;
  ...
  debugptr += sprintf(debugptr, "%i\t%i\t",
                Switch.id,
                Switch.det_block);
  printNew_link(&debugptr, Switch.App);
  printNew_link(&debugptr, Switch.Str);
  printNew_link(&debugptr, Switch.Div);
...
void printNew_MSSwitch(struct configStruct_MSSwitch Switch){
  char debug[1000];
  char * debugptr = debug;
  ...
  for(int i = 0; i < Switch.nr_states; i++){
    ...
    printNew_link(&debugptr, Switch.states[i].sideA);
    printNew_link(&debugptr, Switch.states[i].sideB);
...
void printNew_Signals(struct configStruct_Signal signal){
  char debug[400];
  char * debugptr = debug;

  debugptr += sprintf(debugptr, "%i\t", signal.id);
  printNew_link(&debugptr, signal.block);
...
void printNew_Stations(struct configStruct_Station stations){
  char debug[250];
  char * debugptr = debug;
...
void printNew_Layout(struct configStruct_WebLayout * config){
  printf("Length: %i\n", config->LayoutLength);
  printf("Data:\n%s\n\n", config->Layout);
}

void newModuleConfig::print(char ** cmds, uint8_t cmd_len){
  uint16_t mask = 0;
  ...
  if(mask & 32){
    ...
    for(int i = 0; i < header->MSSwitches; i++){
      printNew_MSSwitch(MSSwitches[i]);
    }
  }
  ...
  if(mask & 128){
    ...
    for(int i = 0; i < header->Stations; i++){
      printf("%i\t", i);
      printNew_Stations(Stations[i]);
    }
  }

  if(mask & 256){
    printf( "Layout\n");
    printNew_Layout(Layout);
  }
}
