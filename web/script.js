...
function create_track(setup){
  ...
  for(var i = 2;i<end1;i++){
    ...
    if(setup[i] == 0){
      ...
      if(segments[setup[i]][1] != 0){
        var M = setup[i];
        $.each(segments[setup[i]][1], function(j){
      ...
      if(r == 0){
        x += segments[setup[i]][5];
        y -= segments[setup[i]][6];
        max_x = Math.max(max_x,x+segments[setup[i]][2]);
        max_y = Math.max(max_y,y+segments[setup[i]][3]);
        min_x = Math.min(min_x,x);
        min_y = Math.min(min_y,y);
      }else if(r == 90){
        x += segments[setup[i]][6];
        y += segments[setup[i]][5];
        max_x = Math.max(max_x,x);
        max_y = Math.max(max_y,y+segments[setup[i]][2]);
        min_x = Math.min(min_x,x-segments[setup[i]][3]);
        min_y = Math.min(min_y,y);
      }else if(r == 180){
        x -= segments[setup[i]][5];
        y += segments[setup[i]][6];
        ...
        min_x = Math.min(min_x,x-segments[setup[i]][2]);
        min_y = Math.min(min_y,y-segments[setup[i]][3]);
      }
      if(segments[setup[i]].length > 11){
        anchor3_r = r + segments[setup[i]][9];
        anchor3_x = x + segments[setup[i]][10];
        anchor3_y = y + segments[setup[i]][11];
      }
      ...
      if(r == 0){
        x += segments[setup[i]][7];
        y += segments[setup[i]][8];
      }else if(r == 90){
        x -= segments[setup[i]][8];
        y += segments[setup[i]][7];
      }else if(r == 180){
        x -= segments[setup[i]][7];
        y -= segments[setup[i]][8];
      }
      r += segments[setup[i]][4];
      if(i == 2 && r == 90){
        r = 0;
      };
      //console.log("M"+setup[i]+" x:"+x+", y:"+y+", R:"+r);
...
$(document).ready(function(){
  ...
  $.ajax({ type: "GET",
     url: './img/F_Cab_Light_Disable.svg',
  ...
  $.ajax({ type: "GET",
     url: './img/F_Cab_Light_Enable.svg',
     success : function(text)
     {
         $('#T1200 .prim_fn').append(text.documentElement);
     }
  ...
  $.ajax({ type: "GET",
     url: './img/F_Cab_Light_Unavailable.svg',
     success : function(text)
     {
         $('#T1200 .prim_fn').append(text.documentElement);
     }
  ...
  $.ajax({ type: "GET",
    ...
    $.ajax({ type: "GET",
       url: './img/F_Default_Unavailable.svg',
       success : function(text)
       {
    ...
    $.ajax({ type: "GET",
       url: './img/F_Horn_Unavailable.svg',
       success : function(text)
       {
           $('#T1200 .secu_fn div div').append(text.documentElement);
       }
    ...
    $.ajax({ type: "GET",
       url: './img/F_Light_Unavailable.svg',
       success : function(text)
       {
           $('#T1200 .secu_fn div div').append(text.documentElement);
       }
    ...
    $.ajax({ type: "GET",
       url: './img/F_Whistle_Unavailable.svg',
       success : function(text)
       {
           $('#T1200 .secu_fn div div').append(text.documentElement);
       }
  ...
  $('#MTrain').children().each(function (index,element){
    if(enable == 0 || $('#MTrain > div').length == index){
      return true;
    }
